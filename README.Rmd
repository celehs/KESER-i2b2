...
* Co-occurrence matrix file, such as `rpdr_code_cooccurrence_victor_2019.csv`

* Co-occurrence matrix dict file (optional), such as `UPMC_AD_wordindex_mapping_datainfo.csv`

* Co-occurrence matrix code frequency counts file (optional), such as `UPMC_AD_code_freq.csv`
...
head(CO_EXAMPLE_CODE)
```

The first two columns are the names of code. The third column is the count of the code pair.

#### Co-occurrence matrix file (with index):

```{r eval=TRUE, echo=FALSE}
head(CO_EXAMPLE_INDEX)
```

The first two columns are the location indexes of code. The third column is the count of the code pair.

#### Co-occurrence matrix dict file:

```{r eval=TRUE, echo=FALSE}
head(CO_DICT_EXAMPLE)
```

The first column is the name of code, the second column is the location index. Only use when the input is **Co-occurrence matrix file with index**.

#### Co-occurrence matrix code frequency counts file:

```{r eval=TRUE, echo=FALSE}
head(CODE_FREQ_EXAMLPE)
```

The first column is the name of code, the second column is the frequency counts.
...
If your Co-occurrence matrix file has code pairs, a dictionary is not needed:
...
Otherwise, if there's no code pairs but index, a dictionary `CO_dict_file` is needed: 
...
Moreover, to further reduce the time cost, a code frequency counts file can be used to remove codes with low frequency. The default cutoff is set to **1000**. To change the cutoff, please refer to documents of function `get_eval_embed`.
...
Let's say you picked the best `dim` from the plots, and the value is **200**. the way the retrieve it is:
